...
def main(preset_name: str = "S0", batch=8, dataset: str = "L_bracket", data_perc: int = 100) -> None:
    # preset_name = "S0"
    # batch = 8
    ...
    save_path = save_dir / f"{geom_prefix}_{data_perc}p_{base_name}_{arch_hash}.pt"
    ...
    # If data_perc is not 100, subsample train_idx accordingly
    if data_perc < 100:
        set_seed(42)
        n_train = len(train_idx)
        n_subsample = max(1, int(n_train * data_perc / 100))
        train_idx = random.sample(train_idx, n_subsample)
        print(f"Subsampling training data to {data_perc}%: {n_subsample} geometries")
